...
const Menu = () => {
    ...
    // Listen for Space key press three times to open password prompt
    useEffect(() => {
        const handleKeydown = (e) => {
            if (e.code === "Space") {
                setSpaceTapCount((prevCount) => {
                    if (prevCount === 2) {
                ...
                // Reset count if there’s a delay between presses (1 second timeout)
                setTimeout(() => {
                    setSpaceTapCount(0);
                }, 1000);
            }
